  ...
  return (
    <main className="grid min-h-screen place-items-center bg-paper px-4">
      <form onSubmit={onSubmit} className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <label className="mt-6 block text-sm font-bold">Full name</label>
        <input className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2" name="full_name" required />
        ...
        <input className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2" name="password" type="password" minLength={6} required />
        {error ? <p className="mt-4 rounded-md bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p> : null}
        <button className="mt-6 w-full rounded-md bg-brand px-4 py-3 font-bold text-white hover:bg-blue-700">Register</button>
        <Link className="mt-4 block text-center text-sm font-semibold text-brand" href="/login">Already have account?</Link>
      </form>
    </main>
  );
}
